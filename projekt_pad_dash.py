...
# dt.head(25)
# Atrybuty x, y, z i carat mają wysokie korelacje, bazują na sobie, table i depth zamieniam na średnią ponieważ jest mała różnica między wierszami,
# przy braku price usuwam, ponieważ są to tylko 4 wartości
dt[['x_dimension', 'y_dimension', 'z_dimension', 'table', 'price', 'depth']] = dt[['x_dimension', 'y_dimension', \
                                                                                    'z_dimension', 'table', 'price', 'depth']].apply(pd.to_numeric)
...
dt['z_dimension'] = dt.apply(lambda row: round((row['x_dimension'] + row['y_dimension']) / 2 * np.mean(z_to_x_and_y_correlation),
                                                2) if np.isnan(row['z_dimension']) else row['z_dimension'], axis=1)
...
# Replace labels to decimal | Zamiana znaczników na dane numeryczne
...
# Cut depth, carat and price attributes outliers to max range of the boxplot |
# Ucinanie wartości odstających dla kategorii depth, carat i price do zakresu wąsów wykresu pudełkowego
...
def update_graph(xaxis_column_name: str, xaxis_type: str) -> dcc.Graph:
    """Update graph.
...
def update_reg_graph(xaxis_column_name: str, xaxis_type: str) -> dcc.Graph:
...
@app.callback(
    ...
    Input('xaxis-column', 'value'))
def boxplot_configuration(xaxis_column: str) -> dcc.Graph:
    """Configure boxplot.

    Parameters
    ----------
    xaxis_column: str
        Chosen attribute to display by interactive dashboard user.

    Returns
    -------
    fig: dcc.Graph
        Indicator boxplots figure.
...
@app.callback(
    ...
    Input('xaxis-column', 'value'))
def histogram_configuration(xaxis_column: str) -> dcc.Graph:
    """Configure histogram.
...
if __name__ == '__main__':
    app.run_server(debug=True)  # Interactive dashboard is running via http://127.0.0.1:8050/
